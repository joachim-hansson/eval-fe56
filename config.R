...
rootpath <- "/home/joape785/eval-fe56-master"
setwd(rootpath)
...
ssh_login <- "joape785@galactica.physics.uu.se"
ssh_pw <- "password"
cluster_node <- "galactica"

# a valid path on the remote machine and the
# local machine, respectively, to store
# result files 
calcdir_rem <- "/home/joape785/eval-fe56-v0-1-0/remcalcdir"
calcdir_loc <- "/home/joape785/calcdir"
...
energyGridrandomFiles <- c(seq(0.1, 30.001, length = 100), seq(30.2, 200.1, length = 100))
...
energyGridForParams <- c(seq(0,31,by=2), 200)
...
outdataPath <- file.path(rootpath, "outdata")
...
pathTalys <-"/home/joape785/t6_new/tasmanPatch/calcarchive"
#savePathTalys <-paste0(cluster_node,":","/TMC/eval-fe56-test/talysResults/defalut_mod_pipeline")
...
createTalysHandlers <- function() {
    ...
    talysHnd <- initClusterTALYS(clustHnd, talysExe = "/home/joape785/t6_new/t6/bin/talys", calcsPerJob = 1000,
