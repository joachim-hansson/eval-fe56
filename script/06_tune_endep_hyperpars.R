...
for(curReac in reactions){
  ...
  gpObsHandler$addGP(expid=curReac, sigma=ifelse(meanCs < 1, 1, meanCs), len=2.5, nugget=1e-1)
}
...
gpDt[grepl("TALYS-",EXPID) & PARNAME == "len", LOWERLIMS := 1]
...
gpDt[grepl("REACEXP",EXPID) & PARNAME == "sigma" , LOWERLIMS := 0.1] # Set different upper limit for GPs on the observable
gpDt[grepl("REACEXP",EXPID) & PARNAME == "len" , LOWERLIMS := 2]# Set different upper limit for GPs on the observable
...
gpDt[grepl("REACEXP",EXPID) & PARNAME == "len" , UPPERLIMS := 50] # Set different upper limit for GPs on the observable
